...
public enum PaymentTypeEnum {
	...
	private static boolean typeContain(String type, PaymentTypeEnum... enums) {
		for (PaymentTypeEnum eType : enums) {
			if (eType.getValue().equals(type)) return true;
		}
		return false;
	}
	...
	public static String getDescByValue(String value) {
		...
		for (PaymentTypeEnum e : values()) {
			if (e.getValue().equals(value)) { return e.getDesc(); }
		}
		return value;
	}
	 ...
	 * @return
	 */
	public static boolean containSQQR(String type) {
		return typeContain(type, SQQR, SQQR_CZ);
	}
}
